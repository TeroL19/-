...
GameGrid::GameGrid(int windowWidth, int windowHeight, int gemSize, int buffer) 
    ...
    calculateSizes();
    grid.resize(gridWidth, std::vector<Gem>(gridHeight));
    init();
}
    ...
    for (int x = 0; x < gridWidth; ++x) {
        ...
        for (int y = gridHeight - 1; y >= 0; --y) {
            if (grid[x][y].getColor() != GemColor::Empty) {
                if (y != changeY) {
                    grid[x][changeY] = grid[x][y];
                    grid[x][y].setColor(GemColor::Empty);
                    changed = true;
                }
                --changeY;
            }
...
void GameGrid::randomGem(int x, int y, const std::vector<GemColor>& exclude) {
    ...
    if (allowed.empty()) {
        grid[x][y] = Gem(GemColor::Empty);
    }
    else {
        std::uniform_int_distribution<int> dist(0, static_cast<int>(allowed.size()) - 1);
        grid[x][y] = Gem(allowed[dist(gen)], BonusType::None, gemSize);
    }
}
...
void GameGrid::removeMatches(SDL_Renderer* renderer, const std::vector<std::pair<int, int>>& matches) {
    for (auto& match : matches) {
        ...
        if (grid[x][y].getBonusType() == BonusType::Colorize) {
            ColorizeBonus currentBonus(currentGem, matches);
            grid[x][y].setColor(GemColor::Empty);
            currentBonus.findGems(grid, x, y);
            currentBonus.renderCycle(renderer, *this);
        }

        else if (grid[x][y].getBonusType() == BonusType::Bomb) {
            BombBonus currentBonus(currentGem, matches);
            grid[x][y].setColor(GemColor::Empty);
            currentBonus.findGems(grid, x, y);
            currentBonus.renderCycle(renderer, *this);
        }
